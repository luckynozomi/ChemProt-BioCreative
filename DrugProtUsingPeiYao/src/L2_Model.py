...
TF_bin_trn = np.array([1*(i != 0) for i in TF_trn])
TF_bin_tst = np.array([1*(i != 0) for i in TF_tst])
...
n_categories = len(relation_to_label_dict)
for Repeat in range(REPEAT):
	...
	n_clf = len(clf_torun)
	prob_trn =  np.zeros([n_clf, n_cases_trn, n_categories])
	pred_Y_trn = np.zeros([n_cases_trn, n_clf])
	Holdout_prob_trn = np.zeros([len(pair_ID_trn),n_clf*n_categories])
	Holdout_class_trn = np.zeros([len(pair_ID_trn),n_clf])
	prob_tst = np.zeros([n_clf, n_cases_tst, n_categories])
	pred_Y_tst = np.zeros([n_cases_tst, n_clf])		
	Holdout_prob_tst = np.zeros([len(pair_ID_tst),n_clf*n_categories])
	...
	if isTrip:
		...
		for k_idx, k in enumerate(pair_ID_trn):
			...
			for m in range(n_clf):
				...
				Holdout_prob_trn[k_idx,(m*n_categories):((m+1)*n_categories)] = prob_tmp[prob_tmp.max(1).argmax(),:]
		Holdout_class_tst = np.zeros([len(pair_ID_tst),n_clf])
		for k_idx, k in enumerate(pair_ID_tst):
			...
			for m in range(n_clf):
				...
				Holdout_prob_tst[k_idx,(m*n_categories):((m+1)*n_categories)] = prob_tmp[prob_tmp.max(1).argmax(),:]
				
	else:
		for m in range(n_clf):
			Holdout_class_trn[:, m] = pred_Y_trn[:,m]
			Holdout_prob_trn[:,(m*n_categories):((m+1)*n_categories)] = prob_trn[m, :, :]
			Holdout_class_tst[:, m] = pred_Y_tst[:,m]
			Holdout_prob_tst[:,(m*n_categories):((m+1)*n_categories)] = prob_tst[m, :, :]
	for i, clf_name in enumerate(clf_torun):
...
average_prob_tst = np.zeros([Holdout_class_tst.shape[0],n_categories])
average_prob_trn = np.zeros([Holdout_class_trn.shape[0],n_categories])	
for m in range(n_clf):
